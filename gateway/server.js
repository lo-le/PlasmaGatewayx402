...
const USDT_ADDRESS = '0xB8CE59FC3717ada4C02eaDF9682A9e934F625ebb'; // USDT0 on Plasma testnet
  ...
  return {
    cryptoPrices: {
      ...
      USDT: {
        price: "1.00",
        change24h: "0.00%",
        volume24h: '$' + (Math.random() * 50 + 100).toFixed(2) + 'B'
      }
    },
    marketData: {
      ...
      plasmaActivity: {
        transactions24h: Math.floor(Math.random() * 100000 + 500000),
        usdtVolume: '$' + (Math.random() * 100 + 500).toFixed(2) + 'M'
      }
    },
  ...
  try {
    ...
    res.status(200).json({
      ...
      payment: {
        ...
        amount: ethers.formatUnits(payment.amount, 6) + ' USDT',
...
app.get('/health', async (req, res) => {
  try {
    ...
    res.json({
      ...
      network: 'Plasma Testnet',
      chainId: 9746,
      price: ethers.formatUnits(price, 6) + ' USDT',
...
// Start server
app.listen(PORT, () => {
  console.log('\n🚀 Plasma x402 Premium Data API Gateway (USDT)');
  console.log('='.repeat(60));
  console.log(`📍 Server: http://localhost:${PORT}`);
  console.log(`📝 Payment Contract: ${CONTRACT_ADDRESS}`);
  console.log(`💵 USDT Token: ${USDT_ADDRESS}`);
  console.log(`🌐 Network: Plasma Testnet`);
  console.log(`💰 Price: $1.00 USDT per request`);
  console.log('='.repeat(60));
  console.log('\n📖 Endpoints:');
  console.log(`   GET  /                       → Frontend UI`);
  console.log(`   GET  /health                 → Health check`);
  console.log(`   GET  /api/premium-data       → Premium data (x402 + USDT)`);
